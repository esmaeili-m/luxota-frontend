...
<template>
  ...
  <div v-else-if="hasPermission">
    <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
      <h1 class="page-title fw-semibold fs-18 mb-0">Trashed Zones</h1>
      <div class="ms-md-1 ms-0">
        <nav>
          <ol class="breadcrumb mb-0">
            <li class="breadcrumb-item"><nuxt-link to="/dashboard/zones">Zones</nuxt-link></li>
            <li class="breadcrumb-item active" aria-current="page">Trashed Zones</li>
          </ol>
        </nav>
      </div>
    </div>
        ...
        <div class="card custom-card">
          <div class="card-header justify-content-between">
            ...
            <div class="prism-toggle">
              <NuxtLink to="/dashboard/zones" class="btn btn-sm btn-primary-light">Back to Zones<i
                  ...
                  <td>
                    <div class="btn-group my-1">
                      ...
                      <ul class="dropdown-menu">
                        <li v-if="auth.user.permissions.includes('zone.restore')"><a
                            class="dropdown-item modal-effect" @click="restoreZone(zone.id)"
                            href="javascript:void(0);">Restore</a></li>
                        <li v-if="auth.user.permissions.includes('zone.delete')"><a data-bs-effect="effect-flip-vertical"  data-bs-toggle="modal" data-bs-target="#delete" class="dropdown-item modal-effect" @click="setZone(zone.id)" href="javascript:void(0);">Delete</a></li>
                      </ul>
                    </div>
                  </td>
                </tr>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h6 class="modal-title" id="deleteModalLabel">Delete Zone</h6>
            <button type="button" class="btn-close" data-bs-dismiss="modal"
                    aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <p>Are you sure you want to delete this zone?</p>
            <p class="text-danger mb-0">Warning: Deleting this zone will also delete all its related data.</p>
          </div>
...
const hasPermission = computed(() => {
  return auth.user?.permissions?.includes('zone.trash') || false
})
...
const { data: zones, pending: isLoadingZones, error, refresh: reloadZones } = await useAsyncData(
    'zones-trash',
    () =>
        $fetch('/zones/trash', {
...
async function confirmDeleteZone() {
  ...
  try {
    const { error } = await useFetch(`/zones/force-delete/${selectedZoneId.value}`, {
      method: 'DELETE',
      baseURL: config.public.apiBase,
      credentials: 'include',
      headers: {
...
async function restoreZone(zoneId) {
  try {
    isLoadingId.value = zoneId;
    await $fetch(`/zones/${zoneId}/restore`, {
      method: 'POST',
      baseURL: config.public.apiBase,
      credentials: 'include',
      headers: {
    ...
    nuxtApp.$toast({
      title: 'success!',
      message: 'Zone Restore successfully.',
      type: 'success',
      duration: 3000
    })
    reloadZones()
