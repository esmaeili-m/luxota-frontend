  ...
  <section class="max-w-7xl mx-auto pb-8 pt-20 dark:bg-dark lg:pb-[70px] lg:pt-[120px]">
    <div class="container px-4 mx-auto">
      <div class="flex justify-between">
        <h2>{{product?.data?.title?.en}}</h2>
        <div>
          {{product?.data?.title?.en}}
        </div>
      </div>
        ...
        <div class="w-2/3">
          <div
           ...
           box-shadow: 6px 6px 12px #010d1d,
                       -6px -6px 12px #011d43;">
            <img
                :src="`${config.public.fileBase}/${product?.data?.image}`"
                :alt="product?.data?.title?.en"
                class="w-full h-full object-cover transition-transform duration-500 group-hover/card:scale-105"
            />
            <div class="p-5">
              <div  v-html="product?.data?.description?.en"  class="leading-loose opacity-70"></div>
            </div>

          </div>
          <div
              class="rounded-3xl overflow-hidden mt-5"
              style="background: linear-gradient(145deg, #01132b, #011633);
           box-shadow: 6px 6px 12px #010d1d,
                       -6px -6px 12px #011d43;">
           <div class="p-6">

             <article v-for="comment in  product?.data?.comments" class="p-6 text-white  rounded-lg ">
               <footer class="flex justify-between items-center mb-2">
                 <div class="flex items-center">
                   <p class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white font-semibold"><img
                       class="mr-2 w-6 h-6 rounded-full"
                       :src="`${config.public.fileBase}/${comment?.user?.avatar}`"
                ...
                <div>
                  ...
                  <div class="mt-2">
                    <div class="w-full">
                      ...
                      <textarea
                          class="p-4 w-full bg-transparent placeholder:text-slate-400  text-white text-sm border border-slate-200 rounded-md px-3 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-300 shadow-sm focus:shadow"
                  ...
                  <div class="mt-2 mb-3">
                    <label class="block text-sm font-medium text-white">Score</label>
                    <div class="flex space-x-2 ">
                      <template v-for="n in 5" :key="n">
                        ...
                        <label
                            ...
                            :class="n <= form.score ? 'text-yellow-400' : 'text-gray-300'"
                        >
                          ★
                        </label>
                      </template>
                    </div>
                    <p v-if="errors.score" class="text-red-500 text-sm mt-1">{{ errors.score }}</p>
...
<!--              <p class="text-[12px]">To obtain the price, please complete the registration process.</p>-->
<!--              <div class="me-3">-->
<!--                <button class="px-4 py-2 text-sm font-medium text-black bg-white-->
<!--                border  border-white rounded-lg  mt-2 cursor-pointer hover:bg-transparent hover:text-white transition">Register Agency</button>-->
<!--              </div>-->
<!--            </div>-->
            <div  class="p-3">
              ...
              <div class="flex items-baseline gap-1 mt-2">
                <p class="text-[35px] font-bold">
                  {{ numberFormat(product?.data?.plans?.six_months?.per_month) }}
                </p>
                <p> {{ product?.data?.currency }}</p>
                <span>/ Month</span>
              </div>
              <div class="mt-1">
                <span class="bg-yellow-500 text-black px-2 py-1 rounded inline-flex items-center gap-2 text-[12px]">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none"
                     xmlns="http://www.w3.org/2000/svg">
                  <path d="M8.5 12.5L10.5 14.5L15.5 9.5" stroke="#000" stroke-width="1.5"
                        stroke-linecap="round" stroke-linejoin="round"/>
                  <path d="M22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C21.5093 4.43821 21.8356 5.80655 21.9449 8" stroke="#000" stroke-width="1.5" stroke-linecap="round"/>
                </svg>
            ...
            <div  class="p-3">
              ...
              <div class="flex items-baseline gap-1 mt-2">
                <p class="text-[26px] font-bold">
                  {{ numberFormat(product?.data?.plans?.three_months?.per_month) }}
                </p>
                <p> {{ product?.data?.currency }}</p>
                <span>/ Month</span>
              </div>
            ...
            <div  class="p-3">
              <div class="flex items-baseline gap-1 mt-2">
                <p class="text-[26px] font-bold">
                  {{ numberFormat(product?.data?.plans?.one_month?.per_month) }}
                </p>
                <p> {{ product?.data?.currency }}</p>
                <span>/ Month</span>
              </div>
          ...
          <div class="p-3 mt-5 rounded-3xl" style="background: linear-gradient(145deg, #01132b, #011633);
           box-shadow: 6px 6px 12px #010d1d,
                       -6px -6px 12px #011d43;">

            <div class="mt-2">
              <h3 class="text-center font-bold">Product Info</h3>
              <hr class="opacity-50">
                <div class="flex justify-between px-4 mt-2">
                  <p class="text-[13px]">Total Subscibee</p>
                  <p class="text-[13px]">{{product?.data?.comments_count}}</p>
                </div>
              <div class="flex justify-between px-4 mt-2">
                  <p class="text-[13px]">Last Update</p>
                  <p class="text-[13px]">{{product?.data?.last_version_update_date}}</p>
                </div>
              <div class="flex justify-between px-4 mt-2">
                  <p class="text-[13px]">Author</p>
                  <p class="text-[13px]">{{product?.data?.author?.name}}</p>
                </div>
              <div class="flex justify-between px-4 mt-2">
                  <p class="text-[13px]">Total Products By Author</p>
                  <p class="text-[13px]">{{product?.data?.author?.products_count}}</p>
                </div>
              <div class="flex justify-between px-4 mt-2">
                  <p class="text-[13px]">Score of product 0/5</p>
                  <p class="text-[13px]">{{ product?.data?.score }}</p>
                </div>
            </div>
...
const validate = () => {
  ...
  if (form.score === 0) {
    errors.score = "Please select a rating.";
    valid = false;
  }
  return valid;
};
...
const {data:product,error,pending,refresh,clear} = useFetch(() => `/products/findBySlug/${route.params.slug}`,{
...
watch(product, (newVal) => {
  if (newVal?.data) {
    form.product_id = newVal.data.id;
  }
}, { immediate: true });
async function handleSubmit() {
  ...
  const { data, error } = await useFetch("/comments", {
    method: "POST",
    baseURL: config.public.apiBase,
    body: form,
    headers: {
