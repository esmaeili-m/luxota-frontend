        ...
        <div class="w-full px-4">
          <div class="mx-auto mb-12 max-w-[485px] text-center lg:mb-[70px]">
              <span class="block mb-2 text-lg font-semibold text-primary">
               {{category?.data?.title?.en}}
              </span>
            <p class="text-base text-body-color dark:text-dark-6">
              {{category?.data?.subtitle?.en}}
            </p>
          </div>
          <!-- Glass Card (Tailwind-only) -->
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 relative">
            <div
                v-for="product in category?.data?.products"
                :key="product.id"
                class="relative rounded-3xl overflow-hidden transition-transform duration-500 transform hover:scale-105 z-0 group/card"
                :class="{
      'blur-[2px] brightness-75': hoverCard !== product.id && hoverCard !== null,
      'shadow-[0_0_10px_rgba(255,255,255,0.7)]  hover:shadow-[0_0_25px_0_rgba(0,255,255,0.7)]': hoverCard === product.id
    }"
                @mouseenter="hoverCard = product.id"
                @mouseleave="hoverCard = null"
            >
              <!-- کارت لینک -->
              <nuxt-link :to="`/market/products/details/${product.slug}`" class="block rounded-3xl overflow-hidden bg-gray-900/80">
                <!-- تصویر -->
                <div class="h-52 w-full overflow-hidden">
                  <img
                      :src="`${config.public.fileBase}/${product?.image}`"
                ...
                <div class="p-4 bg-gray-700">
                  ...
                  <p class="mt-2 text-cyan-400 font-bold">
                    {{ product?.price }} $
                  </p>
                </div>
              </nuxt-link>
...
const {data:category,error,pending,refresh,clear} = useFetch(() => `/categories/findBySlug/${route.params.slug}`,{
